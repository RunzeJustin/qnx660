...
double sinh(double);
 ...
 #else
 #define cos(x)		_CSTD _Sinx(x, 1, 0)
 #endif
 #ifdef _FAST_COSH
 #define cosh(x)	_FAST_COSH(x)
 #else
 #define cosh(x)	_CSTD _Cosh(x, 1.0)
 #endif
 #ifdef _FAST_LOG
 ...
 #else
 #define sin(x)		_CSTD _Sinx(x, 0, 0)
 #endif
 #ifdef _FAST_SINH
 #define sinh(x)	_FAST_SINH(x)
 #else
 #define sinh(x)	_CSTD _Sinh(x, 1.0)
 #endif
#else
...
float sinhf(float);
 ...
 #else
 #define cosf(x)	_CSTD _FSinx(x, 1, 0)
 #endif
 #ifdef _FAST_COSHF
 #define coshf(x)	_FAST_COSHF(x)
 #else
 #define coshf(x)	_CSTD _FCosh(x, 1.0F)
 #endif
 #ifdef _FAST_LOGF
 ...
 #else
 #define sinf(x)	_CSTD _FSinx(x, 0, 0)
 #endif
 #ifdef _FAST_SINHF
 #define sinhf(x)	_FAST_SINHF(x)
 #else
 #define sinhf(x)	_CSTD _FSinh(x, 1.0F)
 #endif
#else
...
long double sinhl(long double);
 ...
 #else
 #define cosl(x)	_CSTD _LSinx(x, 1, 0)
 #endif
 #ifdef _FAST_COSHL
 #define coshl(x)	_FAST_COSHL(x)
 #else
 #define coshl(x)	_CSTD _LCosh(x, 1.0L)
 #endif
 #ifdef _FAST_LOGL
 ...
 #else
 #define sinl(x)	_CSTD _LSinx(x, 0, 0)
 #endif
 #ifdef _FAST_SINHL
 #define sinhl(x)	_FAST_SINHL(x)
 #else
 #define sinhl(x)	_CSTD _LSinh(x, 1.0L)
 #endif
#else
...
__SRCVERSION("$URL: http://svn/product/branches/6.6.0/trunk/lib/m/public/math.h $ $Rev: 735298 $")
#endif
