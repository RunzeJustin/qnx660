...
#define AO_ERRTYPESTR  "libaoi"
/*AO error number*/
typedef enum
{
	AO_EOK = EOK,		/* no error */

	AO_EDLOP = 10000,	/* dlopen error */
	AO_EDLSYM,			/* dlsym error */
	AO_EDUP,				/* add a duplicate DLL */
	AO_EINIT,			/* DLL init call failed */
}AOErrNo_t;
...
int32_t AoSaveConfig(const char * config_file);
int32_t AoLoadConfig(const char * config_file);
const char * const * AoGetStrings(const AOICtrl_t * ctrl);
const char * const * AoGetConfigs(const AOICtrl_t * ctrl);
const AOICtrl_t * AoAddDll(const char * dll, const char * const * configs, AOError_t * const err);
...
__SRCVERSION("$URL: http://svn/product/branches/6.6.0/trunk/lib/aoi/public/aoi/aoi_core.h $ $Rev: 680336 $")
#endif
