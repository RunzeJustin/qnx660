 ...
 * audio manager handle. If @c AUDIO_TYPE_COUNT is specified,
 * the current audio type is used.
 * @param pref_output The preferred output routing of the new
 * audio type. If @c AUDIO_DEVICE_DEFAULT is specified, the
 * current preferred output device is used.
 * @param pref_input The preferred input routing of the new
 * audio type. If @c AUDIO_DEVICE_DEFAULT is specified, the
 * current preferred input device is used.
 *
 * @return @c EOK upon success, negative @c errno upon failure.
 */
...
__SRCVERSION("$URL: http://svn/product/branches/6.6.0/trunk/lib/audioman/public/include/audio/audio_manager_routing.h $ $Rev: 735398 $")
#endif
